...
Function ktn::buildFunction(CXCursor cursor)
{
	...
	Function f(ktn::getFile(cursor), ktn::getFullName(cursor), clang_CXXMethod_isConst(cursor));
	f.name = ktn::convertAndDispose(clang_getCursorSpelling(cursor));
	int num_args = clang_Cursor_getNumArguments(cursor);
	for (int i = 0; i < num_args; ++i) {
		auto arg_cursor = clang_Cursor_getArgument(cursor, i);
		auto name = ktn::convertAndDispose(clang_getCursorSpelling(arg_cursor));
		if (name.empty()) {
	...
	f.returnType = ktn::getName(clang_getResultType(type));
